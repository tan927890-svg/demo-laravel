{{-- resources/views/admin/news/categories.blade.php --}}
...
<div style="padding:32px;max-width:700px">
  ...
  <div style="margin-bottom:24px">
    ...
    <h1 style="font-family:'Bebas Neue';font-size:32px;letter-spacing:2px;color:#f0ebe4">CHUYÊN MỤC TIN TỨC</h1>
  </div>
  ...
  <div style="background:#111;border:1px solid #1c1c1c;padding:24px;margin-bottom:24px">
    <div style="font-size:12px;font-weight:700;letter-spacing:2px;text-transform:uppercase;color:#555;margin-bottom:16px">THÊM CHUYÊN MỤC MỚI</div>
    <form method="POST" action="{{ route('admin.news.categories.store') }}">
      @csrf
      <div style="display:flex;gap:10px">
        <input type="text" name="name"
               placeholder="Tên chuyên mục (vd: Ra Mắt Mới)"
    ...
    <table style="width:100%;border-collapse:collapse">
      ...
      <tbody>
        ...
        <tr style="border-bottom:1px solid #1a1a1a">
          <td style="padding:14px 20px;font-size:13px;color:#f0ebe4">{{ $cat->name }}</td>
          <td style="padding:14px 20px;font-size:12px;color:#555">{{ $cat->slug }}</td>
          <td style="padding:14px 20px;text-align:center;font-size:13px;color:#888">{{ $cat->news_count }}</td>
          <td style="padding:14px 20px;text-align:center">
            <form method="POST" action="{{ route('admin.news.categories.destroy', $cat) }}"
