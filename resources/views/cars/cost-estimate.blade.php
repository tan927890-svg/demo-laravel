...
.dtt-page { background: #f2f1ed; min-height: 100vh; font-family: 'Barlow', sans-serif; }

.page-breadcrumb {
  background: #fff; border-bottom: 1px solid #DDD0B5;
  padding: 13px 40px; display: flex; align-items: center; gap: 8px;
...
.dtt-hero {
  background: var(--bg1); padding: 56px 40px 48px; text-align: center;
  position: relative; overflow: hidden;
}
.dtt-hero::before {
  content: 'DỰ TOÁN'; position: absolute; top: -20px; left: 50%;
...
.dtt-hero-badge::before, .dtt-hero-badge::after { content:''; width:32px; height:1px; background:var(--red); }
.dtt-hero-title {
  font-family: 'Barlow Condensed', sans-serif;
  font-size: clamp(36px, 5vw, 64px); font-weight: 900;
  color: var(--white); text-transform: uppercase;
  letter-spacing: -2px; line-height: .95; position: relative; z-index: 1;
}
...
.dtt-main {
  ...
  display: grid; grid-template-columns: 480px 1fr; gap: 48px; align-items: start;
}

.dtt-form-panel {
  background: #fff; border-top: 3px solid var(--red); border-radius: 14px;
  box-shadow: 0 4px 32px rgba(0,0,0,.10); padding: 40px;
}
...
.dtt-form-heading {
  ...
  color: #111; text-transform: uppercase; letter-spacing: -0.5px; margin-bottom: 28px;
}
...
.dtt-select {
  ...
  border: 1.5px solid #d0cec8; background: #f5f4f0; border-radius: 10px;
...
.dtt-submit-btn {
  width: 100%; padding: 16px; background: var(--red); border: none; cursor: pointer;
  border-radius: 10px; font-family: 'Rajdhani', sans-serif; font-size: 13px; font-weight: 700;
...
/* ── RESULT PANEL ── */
.dtt-result-panel { position: sticky; top: 20px; }
.dtt-result-car-thumb {
  background: var(--bg1); position: relative; overflow: hidden;
  height: 200px; display: flex; align-items: center; justify-content: center;
}
.dtt-result-car-thumb img { width: 100%; height: 100%; object-fit: cover; display: block; opacity: .85; }
.dtt-result-car-thumb-overlay {
  position: absolute; inset: 0;
  background: linear-gradient(to top, rgba(0,0,0,.8) 0%, transparent 60%);
}
.dtt-result-car-info { position: absolute; bottom: 0; left: 0; right: 0; padding: 20px 24px; }
...
.dtt-result-car-variant { font-size: 12px; color: rgba(255,255,255,.5); margin-top: 2px; }
...
.dtt-row {
  ...
  padding: 14px 0; border-bottom: 1px solid #f0eeea; gap: 20px;
}
.dtt-row:last-child { border-bottom: none; }
.dtt-row-label { font-family: 'Barlow', sans-serif; font-size: 14px; color: #555; flex: 1; }
.dtt-row-label small { display: block; font-size: 12px; color: #999; margin-top: 2px; }
.dtt-row-val {
  font-family: 'Barlow Condensed', sans-serif; font-size: 18px;
  font-weight: 700; color: #222; text-align: right; white-space: nowrap; min-width: 140px;
}
...
.dtt-row.sub { padding: 8px 0 8px 20px; border-bottom: 1px solid #f8f7f5; }
.dtt-row.sub .dtt-row-label { font-size: 13px; color: #777; }
.dtt-row.sub .dtt-row-val { font-size: 15px; font-weight: 600; color: #444; }

.dtt-row.total {
  padding: 20px 0 0; border-bottom: none; margin-top: 8px; border-top: 2px solid #111;
}
...
.dtt-ev-badge svg { width: 11px; height: 11px; }
...
.dtt-empty-state { padding: 48px 0; text-align: center; }
.dtt-empty-icon { width: 56px; height: 56px; margin: 0 auto 16px; opacity: .15; }
.dtt-empty-text {
  font-family: 'Rajdhani', sans-serif; font-size: 13px; font-weight: 700;
...
/* ── CTA ── */
.dtt-cta-row { display: flex; gap: 12px; margin-top: 28px; flex-wrap: wrap; }
.dtt-cta-btn {
  flex: 1; min-width: 160px; padding: 15px 20px; text-align: center;
  font-family: 'Barlow', sans-serif; font-size: 13px; font-weight: 600;
  letter-spacing: 1.5px; text-transform: uppercase; text-decoration: none;
  cursor: pointer; border: none; transition: all .2s; display: inline-block; border-radius: 6px;
}
.dtt-cta-btn.primary { background: var(--red); color: #fff; }
.dtt-cta-btn.primary:hover { background: #000; }
...
.dtt-cta-btn.deposit:hover { background: var(--red); color: #fff; border-color: var(--red); }
.dtt-cta-btn.deposit.visible { display: inline-block; }
...
<div class="dtt-page">
  ...
  <div class="dtt-main">
    ...
    <div class="dtt-form-panel">
          ...
          @if(isset($car))
            @foreach($car->variants as $variant)
              <option value="{{ $variant->id }}"
                      ...
                      data-price-no-battery="{{ $variant->price ?? 0 }}"
                      data-name="{{ $car->name }} {{ $variant->name }}"
                      {{ $loop->first ? 'selected' : '' }}>
            ...
            @if($car->variants->isEmpty())
              <option value="0"
                      data-price="{{ $car->sale_price ?? $car->price_per_day ?? 0 }}"
                      data-price-no-battery="{{ $car->sale_price ?? $car->price_per_day ?? 0 }}"
                      data-name="{{ $car->name }}">
                {{ $car->name }}
              </option>
            @endif
            ...
            @foreach($cars as $c)
              @foreach($c->variants as $v)
                <option value="{{ $v->id }}"
                        ...
                        data-price-no-battery="{{ $v->price ?? 0 }}"
                        data-name="{{ $c->name }} {{ $v->name }}">
                  {{ $c->name }} {{ $v->name }}
                </option>
              @endforeach
              @if($c->variants->isEmpty())
                <option value="car-{{ $c->id }}"
                        data-price="{{ $c->sale_price ?? $c->price_per_day ?? 0 }}"
                        data-price-no-battery="{{ $c->sale_price ?? $c->price_per_day ?? 0 }}"
                        data-name="{{ $c->name }}">
                  {{ $c->name }}
                </option>
              @endif
      ...
      <div class="dtt-field" id="field-pin" style="display:none;">
      ...
      <div class="dtt-note">
        ...
        <p>Khu vực II: Gồm các TP trực thuộc trung ương (trừ HN, HCM), TP trực thuộc tỉnh và thị xã</p>
        <p>Khu vực III: Gồm các khu vực còn lại</p>
        <p style="margin-top:8px;"><strong>(**) Lưu ý xe điện:</strong> Miễn phí bảo trì đường bộ theo Nghị định 10/2020/NĐ-CP.</p>
      </div>
    </div>
    ...
    <div class="dtt-result-panel">
      ...
      <div class="dtt-result-car-thumb">
        ...
        <div class="dtt-result-car-info">
          ...
          <div class="dtt-result-car-variant" id="result-car-variant">{{ $car->variants->first()?->name ?? '' }}</div>
        </div>
      </div>
      @endif
        ...
        <div id="result-content" style="display:none;">
          ...
          <div class="dtt-row">
            <div class="dtt-row-label">
              Giá công bố <small>Đã bao gồm VAT</small>
            </div>
          ...
          <div class="dtt-row">
            <div class="dtt-row-label">
              Phí đăng ký biển số
              <small id="r-reg-note">Phụ thuộc khu vực đăng ký</small>
            </div>
          ...
          <div class="dtt-row total">
            ...
            <div class="dtt-row-val" id="r-total"><small>*Tạm tính</small></div>
          </div>

          <div class="dtt-disclaimer">
            Bảng tính trên chỉ mang tính chất tham khảo. Các mức phí áp dụng theo quy định hiện hành và có thể thay đổi. Quý khách vui lòng liên hệ Showroom gần nhất để có báo giá chính xác nhất.
          </div>

          <div class="dtt-cta-row">
            @if(isset($car))
              <a href="#" id="btn-deposit" class="dtt-cta-btn deposit">
                ĐẶT CỌC NGAY →
              </a>
...
(function () {
  ...
  // ── Nhận diện xe điện ──
  // VinFast (VF 3/5/6/7/8/9) và Mercedes EQS là xe điện
  function isElectric(text) {
  ...
  // ── Phí đăng ký biển số ──
  // KV1 (HN/HCM): 20.000.000đ | KV2–3: 1.000.000đ
  const REG_FEE = { '1': 20_000_000, '2': 1_000_000, '3': 1_000_000 };
  const REG_NOTE = {
    '1': 'Khu vực I — TP. Hà Nội & TP. HCM',
    '2': 'Khu vực II — TP trực thuộc tỉnh, thị xã',
    '3': 'Khu vực III — Các khu vực còn lại',
  };

  // ── Phí bảo trì đường bộ/năm ──
  // Xe điện: miễn phí (Nghị định 10/2020/NĐ-CP)
  // Xe xăng dưới 9 chỗ: 1.560.000đ/năm
  const ROAD_FEE_GAS      = 1_560_000;
  const ROAD_FEE_ELECTRIC = 0;

  // ── Bảo hiểm TNDS bắt buộc ──
  // Xe con ≤9 chỗ, 1 năm, đã gồm 10% VAT (theo TT 04/2021/TT-BTC)
  const INSURANCE = 530_900;

  // ── Phí đăng kiểm lần đầu ──
  // Xe con (theo TT 238/2016/TT-BTC)
  const INSPECTION = 560_000;
  ...
  @if(isset($car))
  const depositRouteBase = '{{ route("deposits.create", $car->slug) }}';
  @else
  const depositRouteBase = null;
  @endif

  // Khi đổi tỉnh → tự động chọn khu vực tương ứng
  window.onProvinceChange = function () {
    ...
    if (zone) {
      for (let i = 0; i < zSel.options.length; i++) {
        if (zSel.options[i].value === zone) { zSel.selectedIndex = i; break; }
      }
    }
  };

  // Khi đổi xe → cập nhật tên variant + ẩn/hiện field Pin
  window.onCarChange = function () {
    ...
    if (variantEl && carOpt) variantEl.textContent = carOpt.text || '';

    // Chỉ hiện field Pin khi là xe điện
    const pinField = el('field-pin');
    if (pinField) {
      const ev = isElectric(carOpt?.text || '');
      pinField.style.display = ev ? 'block' : 'none';
      // Reset về mặc định khi ẩn
      if (!ev) el('sel-pin').value = 'included';
    }
  };

  // Chạy ngay lúc load để sync trạng thái ban đầu
  window.onCarChange();
  ...
  window.recalculate = function () {
    ...
    const electric   = isElectric(carOpt.text);
    const regFee     = REG_FEE[zone]  ?? 1_000_000;
    const roadFee    = electric ? ROAD_FEE_ELECTRIC : ROAD_FEE_GAS;
    const tbAmount   = carPrice * rate;
    const total      = carPrice + tbAmount + regFee + roadFee + INSURANCE + INSPECTION;
    ...
    // Phí biển số
    el('r-reg-fee').textContent    = fmt(regFee);
    const regNote = el('r-reg-note');
    if (regNote) regNote.textContent = REG_NOTE[zone] || '';

    // Phí bảo trì đường bộ — xe điện miễn phí
    const roadEl = el('r-road-fee');
    if (electric) {
      roadEl.textContent  = 'Miễn phí';
      roadEl.className    = 'dtt-row-val free';
    } else {
      roadEl.textContent  = fmt(ROAD_FEE_GAS);
      roadEl.className    = 'dtt-row-val';
    }

    el('r-insurance').textContent  = fmt(INSURANCE);
    el('r-inspection').textContent = fmt(INSPECTION);
    el('r-total').innerHTML        = fmt(total) + '<small>*Tạm tính</small>';

    // Cập nhật tên variant trên card ảnh
    const variantEl = el('result-car-variant');
    if (variantEl) variantEl.textContent = carOpt.text || '';

    // Nút đặt cọc
    const btnDeposit = el('btn-deposit');
    if (btnDeposit && depositRouteBase) {
      btnDeposit.href = depositRouteBase + '?rolling_price=' + Math.round(total);
      btnDeposit.classList.add('visible');
    }
  };
